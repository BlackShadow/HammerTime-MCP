...
- Documents: tools act on the active document unless `documentId`, `path` or `documentIndex` selects another open one for that call (`documents_list` shows them). `leaks_load_pointfile` uses `path` for the pointfile itself, so select its document with `documentId`/`documentIndex`. `viewport_capture`, `viewport_focus` and the camera tools work on the on-screen viewports, which always show the active document. `documents_close` refuses a document with unsaved changes unless `force:true` (no save prompt is shown); save it first with `documents_save`.
- Targets: `ids` may name solids, entities or groups; an entity or group target means its brushes (and, for texture tools, their faces). Tools that reshape or delete never fall back to the whole map: `clip_apply`, `clip_split` and `vertex_move` need `ids` or a selection (`vertex_move` also takes the solids named by `vertexRefs`), and `objects_delete` and `objects_transform` need `ids`. Face tools (`texture_*`, `face_*`, `vertex_face_edit`) take `ids`, `objectId` with `faceId`/`faceIds`, `faceRefs`, a selection of objects, or faces picked with `face_select`; their results report the `scope` used.
...
- `undo`/`redo` reverse only the newest change of the document, and only when that change was made through this MCP history; if the user edited the map since, the call is refused rather than corrupting the editor's own undo stack. As in the editor, selection changes (`selection_*`, `viewport_clear_marks`) are undo steps too, and any new change, a selection included, discards what could be redone; check `history_list` before relying on `undo`/`redo`.
